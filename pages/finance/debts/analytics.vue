<template>
  <div class="pb-8">
    ...
    <div class="bg-white rounded-2xl shadow-sm mb-6 p-1.5 flex gap-1.5 overflow-x-auto">
      ...
      <!-- SS-DEV (2026-09-26), 13-band: "SMS" havolasi olib tashlandi — SMS boshqaruvi Tariflar sahifasida -->
      <nuxt-link :to="localePath({ name: 'finance-debts-analytics' })" class="flex items-center gap-2 px-4 py-2.5 rounded-xl font-medium text-sm whitespace-nowrap bg-blue-600 text-white">
...
export default {
  ...
  computed: {
    ...
    texts() {
      ...
      const t = {
        ...
        en: { title: 'Debt analytics', subtitle: 'Detailed statistics and charts on debts', monthlyTrend: 'Monthly debt trend', statusDist: 'Distribution by status', topDebtors: 'Top debtors', paymentTrend: 'Payment trend', lent: 'Lent', borrowed: 'Borrowed', noData: 'No data', totalLent: 'Total lent', totalBorrowed: 'Total borrowed', remaining: 'Remaining debt', overdue: 'Overdue', active: 'Active', completed: 'Completed', cancelled: 'Cancelled' },
        kaa: { title: 'Qarız analitikası', subtitle: 'Qarızlar boyınsha tolıq statistika hám grafikler', monthlyTrend: 'Aylıq qarız dinamikası', statusDist: 'Halatı boyınsha bólistiriw', topDebtors: 'Top qarızdarlar', paymentTrend: 'Tólemler dinamikası', lent: 'Berilgen', borrowed: 'Alınǵan', noData: 'Maǵlıwmat joq', totalLent: 'Jámi berilgen', totalBorrowed: 'Jámi alınǵan', remaining: 'Qalǵan qarız', overdue: 'Múddeti ótken', active: 'Belsendi', completed: 'Tamamlanǵan', cancelled: 'Biykar etilgen' },
      };
      return t[l] || t.uz;
    },
  },
  ...
  methods: {
    ...
    exportDebts() {
      if (!this.checkFeature('excel_export')) return;
      window.open(this.$axios.defaults.baseURL + '/finance/export/debts-excel', '_blank');
    },

    exportPdf() {
      if (!this.checkFeature('pdf_report')) return;
      window.print();
    },
  },
};
