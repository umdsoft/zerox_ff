        ...
        <div class="MyPractices__contents flex flex-col lg:flex-row gap-4 lg:gap-8">
          ...
          <div class="MyPractices__content w-full lg:w-[38%]">
            <div class="MyPractices__title">
              ...
              <nuxt-link :to="localePath({ name: 'jonatuvchi', query: { status: 1 } })">{{ $t("mobil.all")
                }}</nuxt-link>
            </div>
            ...
            <div v-if="data != null" class="space-y-3">
              <div class="MyPractices__cart" v-for="(item, index) in data" :key="index">
                <div class="MyPractices__txt" v-if="item.utype == 2">
                  ...
                  <span v-if="item.type == 2 && $i18n.locale == 'ru'">Перевод на мобильный счет пользователя ({{
                    item.dname }})</span>
                  <span v-if="item.type == 3 && $i18n.locale != 'ru'">{{ item.dname }} {{ $t("mobil.phon2") }}</span>
                  <span v-if="item.type == 3 && $i18n.locale == 'ru'">Перевод с мобильного счета пользователя ({{
                    item.dname }})</span>
          ...
          <div class="MyPractices__content w-full lg:w-[63%] lg:ml-8">
            ...
            <div class="MyPractices__FreeContracts" v-if="line != 0">
              ...
              <div
                class="MyPractices__FreeContractsCart flex flex-col sm:flex-row gap-3 sm:gap-0 sm:items-center sm:justify-between">
                <div class="MyPractices__FreeContractsNum"><span>{{ line }}</span></div>
                <div class="MyPractices__FreeContractsLine sm:w-[80%] w-full">
                  <span
                    :class="{ line1: line == 1, line2: line == 2, line3: line == 3, line4: line == 4, line5: line == 5 }"></span>
                </div>
              </div>
            </div>
            ...
            <div class="MyPractices__replenish">
              ...
              <div class="MyPractices__replenishCarts grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
                <div @click="paymeModal = true"
                  class="MyPractices__replenishCart w-full min-h-[112px] p-4 sm:p-6 flex items-center justify-center">
                  <img class="h-8 sm:h-10 w-auto object-contain"
                    src="https://cdn.payme.uz/logo/payme_color.png?target=_blank" alt="" />
                </div>
                <div @click="clickModal = true"
                  class="MyPractices__replenishCart w-full min-h-[112px] p-4 sm:p-6 flex items-center justify-center">
                  <img class="h-8 sm:h-10 w-auto object-contain" src="@/assets/img/click2.png" alt="" />
                </div>
                <div @click="mobileModal = true"
                  class="MyPractices__replenishCart w-full min-h-[112px] p-4 sm:p-6 flex items-center justify-center">
...
@media (max-width: 640px) {
  ...
  .MyPractices__UserBalans span:nth-child(1) {
    width: auto;
    margin-right: 8px;
  }
