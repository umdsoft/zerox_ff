            ...
            <tr class="cursor-pointer" v-for="(item, index) in contracts" :key="index" @click="viewFullItem(item)">
              <td>
                <div>
                  ...
                  <nuxt-link :to="{ path: '/user', query: { id: item.creditor_uid } }">{{ item.creditor_name }}
                  </nuxt-link>
                </div>
              </td>
              <td>
...
export default {
  ...
  created() {
    let links = [
      {
        title: "Muddati oz qolgan  (debitor)",
        name: "Muddati oz qolgan  (debitor)",
      },
    ];
  ...
  methods: {
    ...
    async getContracts() {
      ...
      try {
        ...
        const exp = await this.$axios.$get(`/contract/exp-near?type=debitor&currency=${this.$route.query.type}`);
