    ...
    <table class="table-auto  w-full lg:hidden">
      ...
      <table class="table-auto hidden lg:inline">
        ...
        <tbody>
            ...
            <td><nuxt-link :to="{path:'/debt-demand',query:{id: item.id}}">Qarzni qaytarishni talab qilish </nuxt-link></td>
            <td><nuxt-link :to="{
                path:'/debt-extend',
                query:{id:item.id}
            }">Qarzni muddatini uzaytirish </nuxt-link></td>
            <td><nuxt-link :to="{path:'/debt-waiver',query:{id:item.id}}" >Qarzdan voz kechish </nuxt-link></td>
    ...
    async getContracts() {
      try {
        const response = await this.$axios.get(
          `/contract/near?type=debitor&page=${this.page + 1}&limit=${
            this.limit
          }`
        );
