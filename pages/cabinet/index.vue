            ...
            <div class="mt-6 space-y-3">
              ...
              <div class="flex items-center justify-between py-3">
                ...
                <div class="flex items-center gap-1.5">
                  ...
                  <span class="text-sm font-semibold text-gray-900">{{ formattedRating }}</span>
                  <!-- Trend strelkasi: o'sish yashil / kamayish qizil; 3 ketma-ket bo'lsa juft -->
                  <span
                    v-if="ratingArrow"
                    class="flex items-center"
                    :style="{ color: ratingArrow.color }"
                    :title="ratingArrow.up ? (ratingArrow.strong ? 'Ketma-ket 3 marta oshgan' : 'Oshgan') : (ratingArrow.strong ? 'Ketma-ket 3 marta kamaygan' : 'Kamaygan')"
                  >
                    <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 24 24">
                      <path v-if="ratingArrow.up" d="M12 5l7 9H5l7-9z" />
                      <path v-else d="M12 19l-7-9h14l-7 9z" />
                    </svg>
                    <svg v-if="ratingArrow.strong" class="w-3 h-3 -ml-1.5" fill="currentColor" viewBox="0 0 24 24">
                      <path v-if="ratingArrow.up" d="M12 5l7 9H5l7-9z" />
                      <path v-else d="M12 19l-7-9h14l-7 9z" />
                    </svg>
                  </span>
...
export default {
  ...
  computed: {
    ...
    /** Reyting — 0.01 formatda (masalan 27.10) */
    formattedRating() {
      const r = Number(this.user?.rating);
      return Number.isFinite(r) ? r.toFixed(2) : '0.00';
    },

    /** Reyting trend strelkasi — rating_type: 1=↑, 2=↓, 3=⇑ (3 ketma-ket), 4=⇓ (3 ketma-ket) */
    ratingArrow() {
      const t = Number(this.user?.rating_type);
      if (t === 1) return { up: true, strong: false, color: '#16a34a' };
      if (t === 3) return { up: true, strong: true, color: '#16a34a' };
      if (t === 2) return { up: false, strong: false, color: '#dc2626' };
      if (t === 4) return { up: false, strong: true, color: '#dc2626' };
      return null;
    },
