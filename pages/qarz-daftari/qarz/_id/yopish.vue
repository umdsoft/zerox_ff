          ...
          <div class="bg-white rounded-xl shadow-sm p-6 sticky top-4">
            ...
            <div class="space-y-3">
              ...
              <div class="flex justify-between text-sm">
                ...
                <span class="font-semibold text-green-600">- {{ formatMoney(form.summa || 0) }} {{ qarz.valyuta }}</span>
              </div>
              <div class="border-t border-gray-100 pt-3 flex justify-between text-sm">
                ...
                <span class="font-bold" :class="newQoldiq <= 0 ? 'text-green-600' : 'text-gray-900'">{{ formatMoney(newQoldiq) }} {{ qarz.valyuta }}</span>
              </div>
...
export default {
  middleware: 'auth',
  data() { return { qarz: null, loading: true, loadError: false, saving: false, form: { summa: null } }; },
  computed: {
    summaFormatted() {
    ...
    texts() {
      ...
      const t = {
        ...
        ru: {
          title: "Закрыть долг", back: "Назад", qoldiq: "Остаток", formTitle: "Данные платежа",
        ...
        kr: {
          title: "Қарзни ёпиш", back: "Орқага", qoldiq: "Қолдиқ", formTitle: "Тўлов маълумотлари",
  ...
  methods: {
    ...
    goBack() {
      ...
      // ($router.push qilsak, history qatlamlanib loop yuzaga keladi)
    ...
    async submit() {
      ...
      try {
        const res = await this.$axios.$post(
          ...
          { summa: s, valyuta: this.qarz.valyuta },
          { silent: true }
        );
