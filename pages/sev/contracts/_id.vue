<template>
  <div>
    <div class="rounded-xl bg-white px-8 py-8 block" v-if="contract != null">
      <div class="flex  justify-between w-100 h-16">
        ...
        <div class="bts">
          ...
          <div class="bottom-actions grid grid-cols-2 gap-6 mb-4">
            ...
            <a :href="`https://pdf.zerox.uz/index.php?id=${contract.uid}&lang=${$i18n.locale}&download=1`" download
              class="
                rounded-lg
                justify-center
                py-2.5
            ...
            <a :href="`https://pdf.zerox.uz/index.php?id=${contract.uid}&lang=${$i18n.locale}&download=1&all=1`"
              download class="
                rounded-lg
                justify-center
                py-2.5
      ...
      <br>
      <div>
        <p><b>{{ $t('list.debitor') }} (Debitor):</b> {{ contract.debitor_name }}</p>
        <p><b>{{ $t('list.creditor') }} (Kreditor):</b> {{ contract.creditor_name }}</p>
        <p><b>{{ $t('action.a11') }}:</b> {{ contract.amount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ") }}
          {{ contract.currency }}</p>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="(item, index) in acts" :key="item.id">
              ...
              <td style="text-align:center;">{{ item.refundable_amount == 0 ? '-' :
                item.refundable_amount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ") }} <span
                  v-if="item.refundable_amount != 0">{{ contract.currency }}</span></td>

              <td style="text-align:center;">{{ item.vos_summa == 0 ? '-' :
                item.vos_summa.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ") }} <span v-if="item.vos_summa != 0">{{
                  contract.currency }}</span></td>
              <td style="text-align:center;">{{ item.residual_amount == 0 ? '-' :
                item.residual_amount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ") }} <span
                  v-if="item.residual_amount != 0">{{ contract.currency }}</span></td>
              ...
              <td v-if="item.res_name == null">Tizim tomonidan {{ dateFormat(item.created_at) }} 23:59 da rad qilindi.
              </td>
            </tr>
