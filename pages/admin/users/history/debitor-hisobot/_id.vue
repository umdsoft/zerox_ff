          ...
          <table v-if="users != null">
            ...
            <tbody>
              <tr v-for="(item, index) in users" :key="index">
                ...
                <td>
                    <span v-if="item.status == '2'">Tugallangan</span>
                    <span v-if="item.status == '3' || item.status == '4'">Rad qilingan</span>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    methods: {
      ...
      async getData() {
        let users = await this.$axios.$get(
          `/dashboard/con/hisobot/${this.$route.params.id}?page=${
            this.page + 1
          }&limit=${this.limit}&type=debitor`
        );
