  ...
  <div class="min-h-screen">
    ...
    <div v-if="contracts.length > 0" class="bg-white rounded-2xl shadow-sm overflow-hidden">
      ...
      <div class="hidden md:grid grid-cols-12 items-center px-6 py-4 bg-gray-50 text-sm font-semibold text-gray-600 border-b border-gray-100">
        ...
        <div class="col-span-2">{{ $t('debt_list.debtsumm') }}</div>
        <div class="col-span-2">{{ $t('debt_list.debta') }}</div>
        <div class="col-span-2">{{ $t('debt_list.debtol') }}</div>
        <div class="col-span-2">{{ $t('debt_list.debtc') }}</div>
      </div>
        ...
        <div v-for="(item, index) in contracts" :key="index" @click="viewFullItem(item)"
          ...
          <div class="hidden md:grid grid-cols-12 items-center">
            <div class="col-span-4 flex items-center gap-3 min-w-0">
              ...
              <span class="inline-block w-3 h-3 rounded-full bg-red-500 ring-4 ring-red-100" v-else-if="item.status == 3 || item.status == 4"></span>
              <span class="inline-block w-3 h-3 rounded-full bg-green-500 ring-4 ring-green-100" v-else></span>
          ...
          <div class="md:hidden">
            <div class="flex items-center justify-between mb-3">
              <div class="flex items-center gap-2">
                ...
                <span class="inline-block w-3 h-3 rounded-full bg-green-500 ring-2 ring-green-100" v-else></span>
                <nuxt-link :to="localePath({ name: 'user', query: { id: item.duid } })"
            ...
            <div class="grid grid-cols-2 gap-3">
              ...
              <div class="bg-orange-50 rounded-xl p-3">
                <div class="text-xs text-orange-600 mb-1">{{ $t('debt_list.debtc') }}</div>
...
export default {
  ...
  methods: {
    ...
    async exportExcel(type, fn, dl) {
      ...
      const fileName = fn || `Muddati oz qolgan (kreditor) ${date.toLocaleString().slice(0, 10)}.${type || "xlsx"}`;
