<template>
  <div style="background: #eaf2fb" class="py-6 rounded">
            ...
            <div class="relative w-28 h-28 sm:w-36 sm:h-36 rounded-full bg-blue-50 ring-1 ring-blue-100 shadow-sm flex items-center justify-center overflow-hidden">
              <div class="w-full h-full flex items-center justify-center p-4">
                ...
                <svg
                  ...
                  preserveAspectRatio="xMidYMid meet"
                >
                  <path d="M12 2C9.24 2 7 4.24 7 7c0 .84.21 1.63.58 2.32C6.02 9.86 5 11.15 5 12.5V14c0 .55.45 1 1 1h.5C7.56 16.72 9.58 18 12 18s4.44-1.28 5.5-3H18c.55 0 1-.45 1-1v-1.5c0-1.35-1.02-2.64-2.58-3.18.37-.69.58-1.48.58-2.32 0-2.76-2.24-5-5-5z" fill="#E91E8C"/>
                  <path d="M12 19c-2.67 0-8 1.34-8 4v1h16v-1c0-2.66-5.33-4-8-4z" fill="#E91E8C"/>
                </svg>
          ...
          <div class="debtor-sum flex justify-between rounded-xl bg-white p-5 h-full">
            ...
            <div class="iconn">
              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M24 48C37.2548 48 48 37.2548 48 24C48 10.7452 37.2548 0 24 0C10.7452 0 0 10.7452 0 24C0 37.2548 10.7452 48 24 48Z" fill="#3182CE" />
                <rect width="48" height="48" rx="8" fill="#3182CE" />
                <path d="M17.1967 17.5676H31.2592C31.4234 17.5675 31.5875 17.5779 31.7505 17.5988C31.6953 17.2111 31.5621 16.8386 31.359 16.5037C31.1559 16.1689 30.8871 15.8786 30.5688 15.6505C30.2506 15.4223 29.8894 15.261 29.507 15.1762C29.1247 15.0914 28.7292 15.0849 28.3443 15.1572L16.7792 17.1316H16.766C16.0401 17.2704 15.3945 17.6812 14.9612 18.2799C15.614 17.8155 16.3955 17.5665 17.1967 17.5676Z" fill="white" />
                ...
                <path d="M14.4062 24.4014V20.0288C14.4062 19.0765 14.9336 17.48 16.7639 17.1341C18.3174 16.8428 19.8555 16.8428 19.8555 16.8428C19.8555 16.8428 20.8662 17.5459 20.0312 17.5459C19.1963 17.5459 19.2183 18.6226 20.0312 18.6226C20.8442 18.6226 20.0312 19.6553 20.0312 19.6553L16.7573 23.3687L14.4062 24.4014Z" fill="white" />
    ...
    <!-- Action buttons - dinamik ravishda type ga qarab -->
...
export default {
  ...
  computed: {
    // Route parametridan type ni olish
    searchType() {
      return this.$route.params.type || 'debitor';
    },

    // Debitor yoki creditor ekanligini aniqlash
    isDebitor() {
      return this.searchType === 'debitor';
    },

    // User ismini ko'rsatish - debitor va creditor uchun farqli
    displayName() {
      if (!this.user) return '';
      if (this.user.type === 1) {
        return this.user.company;
      }
      // Debitor versiyasida user.name, creditor versiyasida to'liq ism
      if (this.isDebitor) {
        return this.user.name || `${this.user.last_name || ''} ${this.user.first_name || ''} ${this.user.middle_name || ''}`.trim();
      }
