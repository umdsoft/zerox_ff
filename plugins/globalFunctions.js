export default ({ app }, inject) => {
  ...
  const latinToCyrillic = (text) => {
    const translitMap = {
      ...
      'k': 'к', 'l': 'л', 'm': 'м', 'n': 'н', 'o': 'о', 'p': 'п', 'q': 'қ', 'r': 'р', 's': 'с',
      't': 'т', 'u': 'у', 'v': 'в', 'x': 'х', 'y': 'й', 'z': 'з',
      'Sh': 'Ш', 'Ch': 'Ч', 'Oʻ': 'Ў', 'Gʻ': 'Ғ', 'sh': 'ш', 'ch': 'ч', 'oʻ': 'ў', 'gʻ': 'ғ', 'Yo': 'Ё', 'yo': 'ё', 'Yu': 'Ю', 'yu': 'ю', 'Ya': 'Я', 'ya': 'я', 'ʻ': 'ъ'
    };

    // "yev" bilan tugagan so'zlar uchun maxsus almashtirish
    text = text.replace(/([QGJKHRShY])ev\b/g, function(match, p1) {
      return p1.toLowerCase() === 'q' || p1.toLowerCase() === 'g' ? p1 + 'ев' : p1 + 'ев';
    });
    ...
    return text.replace(/Sh|Ch|Oʻ|Gʻ|Yo|Yu|Ya|sh|ch|oʻ|gʻ|yo|yu|ya|[A-Za-zʻ]/g, function(match) {
