      ...
      <div class="box qarz">
        <div class="content-title">
          ...
          <p>
            ...
            bo'yicha qarz mablag'i olinganligi to'g'risida )
          </p>
        </div>
        <div class="content-body">
          ...
          <p>
            ...
            >ga <b>{{ dateFormat(contract.createdAt) }} </b> yildagi
            <span>{{ contract.number }}</span> -sonli qarz shartnomasiga asosan
            <span>{{
              contract.amount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ")
            }}</span>
            {{ contract.currency }} miqdoridagi pul mablag'ini
            <span> {{ dateFormat(act.end_date) }}</span> yilgacha foizsiz qarz
            sifatida topshirildi.
          </p>
          <p>
