    ...
    <div v-if="$i18n.locale == 'uz'">
      <div v-if="item.debitor === item.reciver">
        ...
        <p class="mt-2">
          ...
          yuborgansiz. Ushbu qarz shartnomasi {{ dateFormat(item.created.slice(0,10)) }} yil
      ...
      <div v-if="item.creditor === item.reciver">
        ...
        <p class="mt-2">
          ...
          so‘rovnoma
          yuborilgan. Ushbu qarz shartnomasi {{ dateFormat(item.created.slice(0,10)) }} yil
    ...
    <div v-if="$i18n.locale == 'kr'">
      <div v-if="item.debitor === item.reciver">
        ...
        <p class="mt-2">
          ...
          {{ dateFormat(item.created.slice(0,10)) }} йилда <a class="text-blue-400"
          ...
          сўровнома юборгансиз. Ушбу қарз шартномаси {{ dateFormat(item.created.slice(0,10)) }} йил соат 23:59 га қадар <b
      ...
      <div v-if="item.creditor === item.reciver">
        ...
        <p class="mt-2">
          <b v-if="item.dtypes == 2">{{ item.d_last_name }} {{ item.d_first_name }} {{ item.d_middle_name }}</b><b v-if="item.dtypes == 1">{{ item.dcompany }}</b>
          томонидан {{ dateFormat(item.created.slice(0,10)) }} йилда <a class="text-blue-400"
            :href="`https://pdf.zerox.uz/index.php?id=${item.uid}&lang=uz&download=0`" target="_blank"><b>{{ item.number
              }}</b></a>-сонли қарз шартномасини расмийлаштириш учун
          Сизга сўровнома юборилган. Ушбу қарз шартномаси {{ dateFormat(item.created.slice(0,10)) }} йил соат 23:59 га қадар Сиз
          томонингиздан қабул
    ...
    <div v-if="$i18n.locale == 'ru'">
      <div v-if="item.debitor === item.reciver">
        ...
        <p class="mt-2">
          Вы направили <b v-if="item.ctypes == 2">{{ item.c_last_name }} {{ item.c_first_name }} {{ item.c_middle_name }}</b><b v-if="item.ctypes == 1">{{
            item.ccopmany }}</b> {{ dateFormat(item.created.slice(0,10)) }}г. запрос на оформление договора займа № <a
            class="text-blue-400" :href="`https://pdf.zerox.uz/index.php?id=${item.uid}&lang=uz&download=0`"
            target="_blank"><b>{{ item.number }}</b></a>.
          Этот договор займа был отклонен системой в связи с тем, что до 23:59 {{ dateFormat(item.created.slice(0,10)) }} г. он не
      ...
      <div v-if="item.creditor === item.reciver">
        ...
        <p class="mt-2">
          ...
          <b v-if="item.dtypes == 2">{{ item.d_last_name }} {{ item.d_first_name }} {{ item.d_middle_name }}</b><b v-if="item.dtypes == 1">{{ item.dcompany }}</b>
          {{ dateFormat(item.created.slice(0,10)) }} г. направил вам анкету на оформление договора займа № <a class="text-blue-400"
            :href="`https://pdf.zerox.uz/index.php?id=${item.uid}&lang=uz&download=0`" target="_blank"><b>{{ item.number
              }}</b></a>.
          Этот договор займа был отклонен системой в связи с тем, что до 23:59 {{ dateFormat(item.created.slice(0,10)) }} г. он не
          было принят Вами.
